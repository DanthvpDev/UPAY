...
BEGIN 
    ...
    ON AJS.DESCRIPCION = I.DESCRIPCION
...
CREATE OR ALTER TRIGGER TR_FECHA_VALIDA_PLANILLA_ORDINARIA 
ON PLANILLAS AFTER INSERT 
AS 
BEGIN

	-- Obtener el último día del mes actual
    DECLARE @ULTIMO_DIA_MES_ACTUAL DATE = EOMONTH(GETDATE());
	DECLARE @SIGUIENTE_MES INT = MONTH(GETDATE()) + 1;
	DECLARE @MES_ACTUAL INT = MONTH(GETDATE());
	
	IF EXISTS(SELECT 1
			  FROM inserted I
			  WHERE MES < @MES_ACTUAL AND TIPO_PLANILLA != 'EXT')
		BEGIN 
			RAISERROR('NO SE PUEDEN AGREGAR PLANILLAS ORDINARIAS DE MESES ANTERIORES', 16, 5)
			ROLLBACK TRANSACTION
		END

	IF EXISTS(SELECT 1
			  FROM inserted I
			  WHERE MES > @SIGUIENTE_MES)
		BEGIN 
			RAISERROR('NO SE PUEDEN AGREGAR PLANILLAS DE MESES MÁS ALLÁ DEL PRÓXIMO MES.', 16, 5)
			ROLLBACK TRANSACTION
		END

	IF EXISTS(SELECT 1
			  FROM inserted I
			  WHERE MES = @SIGUIENTE_MES AND FECHA_CALCULO < @ULTIMO_DIA_MES_ACTUAL)
		BEGIN 
			RAISERROR('NO SE PUEDE AGREGA LA PLANILLA ORDINARIAS DEL MES SIGUIENTE DEBIDO A QUE NO ES EL ÚLTIMO DÍA DEL MES EN CURSO.', 16, 5)
			ROLLBACK TRANSACTION
		END
END
GO
...
BEGIN 
	...
	INNER JOIN inserted I 
	ON AJS.ID = I.ID
	WHERE AJS.DESCRIPCION LIKE '%DEDICACIÓN EXCLUSIVA%' OR AJS.DESCRIPCION LIKE '%CARRERA PROFESIONAL%' OR AJS.DESCRIPCION LIKE '%DEDICACION EXCLUSIVA%'
END
GO
